...
v0.1.0 MVP は local library の作成、arXiv source package import、local TeX archive import、Markdown の弱構造 document import、PDF の source artifact 保存、LaTeXML による TeX parsing、structured document blocks と assets の保存、deterministic local block embeddings、OpenAI-compatible または Anthropic-compatible provider による paragraph/caption translation、translation variants、translation memory review、article glossary、macOS Keychain provider key storage、article-grounded streaming Q&A、editable lecture-note patches、custom note templates、Markdown/bundle export を備えています。
